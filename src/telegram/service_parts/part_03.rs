...
mod tests {
    ...
    async fn telegram_qr_new_user_session_and_bot_start_preserve_identity_and_role() {
        ...
        let account = TelegramUserAccount {
            telegram_user_id: "123".into(), telegram_chat_id: String::new(),
            username: "new_user".into(), display_name: "New User".into(),
            role: TelegramAccountRole::SalesManager, invite_token: String::new(), joined_at_unix: 1,
            phone_number: "998123456789".into(), delivery_mode: TelegramDeliveryMode::UserProfile,
            user_profile_connected: true, selected_chat_id: None, selected_chat_title: None, selected_chat_type: None,
        };
        service.store.register_qr_user(account.clone(), "qr-session-secret".into()).await.unwrap().unwrap();
        let mut duplicate = account.clone();
        duplicate.role = TelegramAccountRole::Admin;
        assert!(service.store.register_qr_user(duplicate, "replacement-session".into()).await.unwrap().is_none());
        let start = || TelegramStartRequest {
            invite_token: String::new(), telegram_user_id: "123".into(), telegram_chat_id: "123".into(),
            username: "new_user".into(), display_name: "New User".into(),
        };
        // The unauthenticated HTTP start route cannot hijack a QR-linked user's chat.
        assert!(matches!(service.register_start(start()).await, Err(TelegramError::InviteTokenRequired)));
        ...
        assert!(matches!(service.register_bot_start(unknown).await, Err(TelegramError::InviteTokenRequired)));
        ...
        assert_eq!(reloaded.store.user_session("123").await.unwrap().as_deref(), Some("qr-session-secret"));
        assert_eq!(reloaded.admin_overview().await.unwrap().users, vec![recognized]);
    }

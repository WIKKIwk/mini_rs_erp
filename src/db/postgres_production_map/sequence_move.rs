...
async fn commit_once(
    ...
    sqlx::query("INSERT INTO mini_queue_sequences (apparatus,canonical_apparatus_id,order_ids,updated_at)
        VALUES (COALESCE((SELECT name FROM mini_apparatus WHERE id=$1),$1),$1,$2,now())
        ON CONFLICT (canonical_apparatus_id) DO UPDATE SET order_ids=excluded.order_ids,updated_at=excluded.updated_at")
        .bind(&command.apparatus).bind(serde_json::json!(result.order_ids))

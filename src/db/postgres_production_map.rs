...
impl ProductionMapStorePort for PostgresProductionMapStore {
    ...
    async fn daily_apparatus_sequences(
        ...
        let rows = sqlx::query_as::<_, (String, String, serde_json::Value)>(
            "SELECT work_date, apparatus, order_ids
             FROM mini_daily_apparatus_sequences
             ORDER BY work_date ASC, apparatus ASC",
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|_| ProductionMapError::StoreFailed)?;
        ...
        for (work_date, apparatus, payload) in rows {
            let order_ids = serde_json::from_value::<Vec<String>>(payload)
                .map_err(|_| ProductionMapError::StoreFailed)?;
            grouped
                .entry(work_date)
    ...
    async fn put_daily_apparatus_sequence(
        ...
        if order_ids.is_empty() {
            sqlx::query(
                "DELETE FROM mini_daily_apparatus_sequences
        ...
        let payload =
            serde_json::to_value(order_ids).map_err(|_| ProductionMapError::StoreFailed)?;
        sqlx::query(
            "INSERT INTO mini_daily_apparatus_sequences
                (work_date, apparatus, order_ids, updated_at)
             VALUES ($1, $2, $3, now())

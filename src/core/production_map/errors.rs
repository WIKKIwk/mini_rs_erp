...
pub enum ProductionMapError {
    ...
    #[error(
        "merge input kadr count {scanned_kadr_count} does not match active input kadr count {active_kadr_count}"
    )]
    ...
    #[error("rezka frame groups must be positive and match kadr count")]
    ...
    #[error("opening WIP target production-map stage is already completed")]
    OpeningWipTargetStageAlreadyCompleted,
